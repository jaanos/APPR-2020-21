...
graf_k <- ggplot(tabela_1) +
  ...
  geom_point() +
  labs(title="Razvrstitev po metodi voditeljev",
...
model <- lm(data=tabela_2, Stevilo~Leto + I(Leto)^2)
...
graf_r <- ggplot(tabela_2, aes(x=Leto, y=Stevilo)) +
  ...
  labs(title="Napoved gibanja števila vseh prenočitev za leto 2021",
